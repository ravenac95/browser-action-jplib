...
function createPopup(browserAction, badgeState) {
    ...
    let { sendMessage, onMessage } = createMessageChannel(popup.contentScriptOptions, popup.port);
    browserAction.onMessage = onMessage;
    browserAction.sendMessage = sendMessage;
    return popup;
}
...
function createBadge(browserAction, badgeState) {
    ...
    var tbw = require('toolbarwidget').ToolbarWidget({
        ...
        onClick: function() {
          popup.port.emit('requestDimensionsUpdate', 'hello');
          browserAction.onClicked.dispatch();
        }
    });
    return { tbw: tbw, popup: popup };
}
...
function Badge(badgeOptions) { // Exported
    ...
    let { tbw, popup } = createBadge(browserAction, badgeState);
    ...
    browserAction.resizePopup = function(width, height) {
      popup.resize(width + 16, height + 30);
    };

    browserAction.hidePopup = function() {
      popup.hide();
    };
